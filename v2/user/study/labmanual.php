...
<body>
            ...
            <div class="row no-gutters slider-text align-items-center justify-content-center" data-scrollax-parent="true">
                <div class="col-md-8 text-center">
                    <p class="breadcrumbs"><span class="mr-2"><a  href="../home/home">Home</a></span> <span>Study</span></p>
                        <h1 class="mb-3 bread">Lab Manuals</h1>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        $sem=$_POST['sem'];
        $code=$_POST['code'];
        $view = $study->search($sem, $code, $dept, "labmanual_tbl");
        ...
        <?php while($data1 = $view->fetch_array()) { ?>
            <div class="alert bg-success text-white col-md-3">
                ...
                <p><?= $data1['topic']; ?></p>
                <button class="btn btn-link" onclick="location.href= 'download?file=<?= $data1['material'] ?>&path=manual'">Download</button>
            </div>
        <?php } ?>
        </div>
