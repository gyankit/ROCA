<?php 
	...
	{
		header("location: labmanual.php");
	}

	$sql1="Select distinct department from labmanual_tbl";
	$rs1=mysqli_query($cn,$sql1);

	$sql2="Select distinct semester from labmanual_tbl";
	$rs2=mysqli_query($cn,$sql2);

	$sql3="Select distinct subject from labmanual_tbl";
	$rs3=mysqli_query($cn,$sql3);

	$sql5="select * from labmanual_tbl order by id desc";
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
	{
		...
		if(($dept=="") and ($sem=="")) {
			$sql5="select * from labmanual_tbl where and subject='$sub'";
		}
		elseif(($dept=="") and ($sub=="")) {
			$sql5="select * from labmanual_tbl where semester='$sem'";
		}
		elseif(($sem=="") and ($sub=="")) {
			$sql5="select * from labmanual_tbl where department='$dept'";
		}
		elseif($dept=="") {
			$sql5="select * from labmanual_tbl where semester='$sem' and subject='$sub'";
		}
		elseif($sem=="") {
			$sql5="select * from labmanual_tbl where department='$dept' and subject='$sub'";
		}
		elseif($sub=="") {
			$sql5="select * from labmanual_tbl where department='$dept' and semester='$sem'";
		}	
		else {
			$sql5="select * from labmanual_tbl where department='$dept' and semester='$sem' and subject='$sub'";
		}
	}
		...
		<div class="jumbotron-fluid">
			...
			<table class="table table-striped">
				...
				<tbody class="table-sm">
					...
					{
						?>
						<script>alert("No Lab Manual Found...");</script>
						<?php
					}
					else
					{
						...
						<tr>
							...
							<td><?php echo $db['manual'];?></td>
							<td><a href="deletemanual.php?id=<?php echo $db['id'];?>&photo=<?php $db['manual']; ?>">Delete</a></td>
						</tr>
						<?php
						}
					}
