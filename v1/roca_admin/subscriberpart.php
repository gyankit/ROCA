			...
			<?php while($db=mysqli_fetch_array($rs)) {
				...
				<div class="alert alert-danger">
					...
					<div class="collapse" id="<?php echo $eid; ?>">
					    <?php 
					    $sql2="select * from participation_tbl where id='$eid' and paid='YES'";
						$sr1=mysqli_query($cn,$sql2);
					    if($sr1==true) { ?>
						<div class="alert alert-success h4 font-weight-bold">
						    Total : <span><?php echo mysqli_num_rows($sr1); ?></span>
							<button class="btn float-right text-danger font-weight-bold" onClick="location.href = 'excel.php?id=subscriber&event=<?php $event; ?>&eid=<?php echo $eid; ?>';">Excel</button>
						</div>
						<br>
