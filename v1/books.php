...
<body>
	...
	<div class="container">
		...
		if($_SERVER["REQUEST_METHOD"]=="POST")
		{
			...
			if($sem=="" and $code=="") {
				$sql2="select * from books_tbl where department='$dept'";
			}
			elseif($code=="") {
				$sql2="select * from books_tbl where semester='$sem'";
			}
			elseif($sem=="") {
				$sql2="select * from books_tbl where code='$code'";
			}
			else {
				$sql2="select * from books_tbl where department='$dept' and semester='$sem' and code='$code'";
			}
			...
			if($rs) 
			{
				if(mysqli_num_rows($rs)!=0)
				{
					...
					<div class="alert alert-success float-left text-center font-weight-bold text-danger">
						...
						<a href="downloadbooks.php?file=<?php echo $db1['books'];?>"><?php echo $db1['books'];?><br>
						<?php echo $db1['link']; ?>
					</div>
					<?php
					}
